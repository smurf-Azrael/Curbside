...
const mockUser = mocks.Users.filter(user => user.id === USER_ID)[0];
const profileImageTEMP = mockUser.photoUrl;
// ---

const nameToDisplay = `${mockUser.firstName} ${mockUser.lastName.slice(0,1)}.`;
const imageUrlArray: string[] = mockListing.photoUrls;
...
const ListingDetailView = () => {
  // Need to add chat button link to chat
  ...
  return (
    <section className='listing-detail-container'>
      ...
      <section className='listing-details-gallery-wrapper'>
        <ImageCarousel carouselItems={imageUrlArray} />
      </section>
      <section className='listing-details-data-wrapper'>
        <h4 style={{fontWeight:"bold"}}>{mockListing.title}</h4>
